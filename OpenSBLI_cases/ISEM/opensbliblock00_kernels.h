...
void instantiate_eddies(ACC<double>& eddy_x, ACC<double>& eddy_y, ACC<double>& eddy_z, ACC<double>& eddy_r, ACC<double>& eddy_increment, ACC<int>& eddy_eps_x, ACC<int>& eddy_eps_y, ACC<int>& eddy_eps_z, const ACC<int>& eddy_x_rng, const ACC<int>& eddy_bulk_rng){
  ...
  xnorm = (double)eddy_bulk_rng(0,0,0,0)/(2147483648.0);
  xnorm_shift = (xnorm + rng_shift) / rng_divfact;
  y_rngshift = rng_minval/pow(xnorm_shift, powfact);

  eddy_x(0,0,0) = eddy_x_min + (eddy_x_rng(0,0,0) + 2147483648.0) / (4294967295.0) * (eddy_x_max - eddy_x_min);
  eddy_y(0,0,0) = eddy_y_min + (eddy_y_max - eddy_y_min) * y_rngshift;
  ...
  eddy_r(0,0,0) = (eddy_y(0,0,0) < 0.1) ? 0.1 : eddy_y(0,0,0)*0.41;
  
  eddy_increment(0,0,0) = (eddy_y(0,0,0) < 0.01 * delta) ? pow(0.01, 0.16666667)*dt 
  : (eddy_y(0,0,0) < delta) ? pow(eddy_y(0,0,0)/delta, 0.166667) * dt : 1.0 * dt;
}


void convect_eddies(ACC<double>& eddy_x, ACC<double>& eddy_y, ACC<double>& eddy_z, ACC<double>& eddy_r, ACC<double>& eddy_increment, ACC<int>& eddy_eps_x, ACC<int>& eddy_eps_y, ACC<int>& eddy_eps_z, const ACC<int>& eddy_bulk_rng){
  double xnorm;
  double xnorm_shift;
  double y_rngshift;
  ...
  if(eddy_x(0,0,0) > eddy_x_max){
    ...
    eddy_increment(0,0,0) = (eddy_y(0,0,0) < 0.01 * delta) ? pow(0.01, 0.1666667) * dt
    : (eddy_y(0,0,0) < delta) ? pow(eddy_y(0,0,0)/delta, 0.166666667) * dt : 1.0 * dt ;
  }
}
...
: (
   ...
   if(x1d < 1.0 && x1d > 0.0){
     for (int i{0}; i < eddies; i++){
       ...
       if(rsq < 1.0){
         shape = 1.0/1.85;///1.85; ///1.7;//1.5829045;
   ...
   Mlocal = u0 / sqrt(T) * Minf;
   
   Tp = (x1d > 0.0 && x1d < 1.0) ? -(gama-1) * Mlocal*Mlocal * T * up / u0 : 0.0; 
