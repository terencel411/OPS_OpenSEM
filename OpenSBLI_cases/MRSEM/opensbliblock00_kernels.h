...
void interp_RST(const ACC<double>& x1_B0, ACC<double>& a11, ACC<double>& a21, ACC<double>& a22, ACC<double>& a33){
...
void instantiate_eddies(ACC<double>& eddy_pos, ACC<double>& eddy_r, ACC<int>& eddy_eps, const ACC<int>& eddy_bulk_rng, const int* regnum){
  eddy_r(0,0,0,0) = radii[3*regnum[0]];
  eddy_r(1,0,0,0) = radii[3*regnum[0]+1];
  eddy_r(2,0,0,0) = radii[3*regnum[0]+2];
  eddy_pos(0,0,0,0) = simulation_time + ((eddy_bulk_rng(0,0,0,0) + 2147483648.0) / (4294967295.0) - 1.0) * eddy_r(0,0,0,0);
  eddy_pos(1,0,0,0) = eddy_y_min[regnum[0]] + (eddy_bulk_rng(1,0,0,0) + 2147483648.0) / (4294967295.0) * (eddy_y_max[regnum[0]] - eddy_y_min[regnum[0]]);
  eddy_pos(2,0,0,0) = eddy_z_min[regnum[0]] + (eddy_bulk_rng(2,0,0,0) + 2147483648.0) / (4294967295.0) * (eddy_z_max[regnum[0]] - eddy_z_min[regnum[0]]);
  eddy_eps(0,0,0,0) = (eddy_bulk_rng(3,0,0,0) < 0) ? -1 : 1;
  eddy_eps(1,0,0,0) = (eddy_bulk_rng(4,0,0,0) < 0) ? -1 : 1;
  eddy_eps(2,0,0,0) = (eddy_bulk_rng(5,0,0,0) < 0) ? -1 : 1;
}


void convect_eddies(ACC<double>& eddy_pos, const ACC<double>& eddy_r, ACC<int>& eddy_eps, const ACC<int>& eddy_bulk_rng, const int* regnum){
  
  if((simulation_time - eddy_pos(0,0,0,0) - eddy_r(0,0,0,0)) / eddy_r(0,0,0,0) > 1.0){
    eddy_pos(0,0,0,0) = simulation_time + (eddy_bulk_rng(0,0,0,0) + 2147483648.0) / (4294967295.0)* eddy_r(0,0,0,0);
    eddy_pos(1,0,0,0) = eddy_y_min[regnum[0]] + (eddy_bulk_rng(1,0,0,0) + 2147483648.0) / (4294967295.0) * (eddy_y_max[regnum[0]] - eddy_y_min[regnum[0]]);
    eddy_pos(2,0,0,0) = eddy_z_min[regnum[0]] + (eddy_bulk_rng(2,0,0,0) + 2147483648.0) / (4294967295.0) * (eddy_z_max[regnum[0]] - eddy_z_min[regnum[0]]);
    eddy_eps(0,0,0,0) = (eddy_bulk_rng(3,0,0,0) < 0) ? -1 : 1;
    eddy_eps(1,0,0,0) = (eddy_bulk_rng(4,0,0,0) < 0) ? -1 : 1;
...
&rhou2_B0, ACC<double> &x0_B0, ACC<double> &x2_B0, ACC<double> &rho_B0, ACC<double> &x1_B0, const int *idx)
{
   ...
   (x1_B0(0,0,0) > 0.0 && x1d < 1.0) ? pow(x1d, 0.166666667) : 1.0 ;
...
: (
 ...
 void opensbliblock00Kernel030(const ACC<double> &x1_B0, const ACC<double>&x2_B0, ACC<double> &rhoE_B0, ACC<double> &rhou0_B0, ACC<double>&rhou1_B0, ACC<double> &rhou2_B0, ACC<double> &rho_B0, const double* eddy_pos1_gbl, const double* eddy_r1_gbl, const int* eddy_eps1_gbl, const double* eddy_pos2_gbl, const double* eddy_r2_gbl, const int* eddy_eps2_gbl, const double* eddy_pos3_gbl, const double* eddy_r3_gbl, const int* eddy_eps3_gbl, const int* idx)
{
   double T = 0.0;
   double p = 0.0;
   double psifn = 0.0;
...
: (
   1.0
    ...
    // --------- SEM interaction with inlet plane ---------------------
   if (x1_B0(0,0,0) > 0.0 && x1_B0(0,0,0) < eddy_y_max[0] + radii[1]){
     for (int i{0}; i < eddies[0]; i++){
       normpos[0] = (simulation_time - eddy_pos1_gbl[3*i] - eddy_r1_gbl[3*i]) / eddy_r1_gbl[3*i];
       normpos[1] = (x1_B0(0,0,0) - eddy_pos1_gbl[3*i+1]) / eddy_r1_gbl[3*i+1];
       normpos[2] = (x2_B0(0,0,0) - eddy_pos1_gbl[3*i+2]) / eddy_r1_gbl[3*i+2];
       
       rsq = normpos[0]*normpos[0] + normpos[1]*normpos[1] + normpos[2]*normpos[2];
       if(rsq < 1.0){
         shape[0] = 1.0/1.4;//1.4;//1.7;
         shape[0] *= exp(-0.5*normpos[0]*normpos[0]);
         shape[0] *= exp(-0.5*normpos[1]*normpos[1]);
         shape[0] *= exp(-0.5*normpos[2]*normpos[2]);
         
         shape[1] = 1.0/1.4;//1.4;//1.7;
         shape[1] *= exp(-0.5*normpos[0]*normpos[0]);
         shape[1] *= exp(-0.5*normpos[1]*normpos[1]);
         shape[1] *= exp(-0.5*normpos[2]*normpos[2]);
         
         shape[2] = 1.0/1.4;//1.4;//1.7;
         shape[2] *= exp(-0.5*normpos[0]*normpos[0]);
         shape[2] *= exp(-0.5*normpos[1]*normpos[1]);
         shape[2] *= exp(-0.5*normpos[2]*normpos[2]);
         
         up += a11[idx[1]] * eddy_eps1_gbl[3*i] * shape[0];
         vp += (a21[idx[1]] * eddy_eps1_gbl[3*i] * shape[0] + a22[idx[1]] * eddy_eps1_gbl[3*i+1] * shape[1]);
         wp += a33[idx[1]] * eddy_eps1_gbl[3*i+2] * shape[2];
       }
       else{ // periodic boundary condition
  
         normz_ghost = (x2_B0(0,0,0) < 20.0) ? (x2_B0(0,0,0)+40.0 - eddy_pos1_gbl[3*i+2]) / eddy_r1_gbl[3*i+2] : (x2_B0(0,0,0)-40.0 - eddy_pos1_gbl[3*i+2]) / eddy_r1_gbl[3*i+2];
         rsq = rsq - normpos[2]*normpos[2] + normz_ghost*normz_ghost;
         if(rsq < 1.0){
           shape[0] = 1.0/1.4;//1.4;//1.7;
           shape[0] *= exp(-0.5*normpos[0]*normpos[0]);
           shape[0] *= exp(-0.5*normpos[1]*normpos[1]);
           shape[0] *= exp(-0.5*normz_ghost*normz_ghost);
           
           shape[1] = 1.0/1.4;//1.4;//1.7;
           shape[1] *= exp(-0.5*normpos[0]*normpos[0]);
           shape[1] *= exp(-0.5*normpos[1]*normpos[1]);
           shape[1] *= exp(-0.5*normz_ghost*normz_ghost);
           
           shape[2] = 1.0/1.4;//1.4;//1.7;
           shape[2] *= exp(-0.5*normpos[0]*normpos[0]);
           shape[2] *= exp(-0.5*normpos[1]*normpos[1]);
           shape[2] *= exp(-0.5*normz_ghost*normz_ghost);
   ...
   if(x1_B0(0,0,0) > eddy_y_min[1] - radii[4] && x1_B0(0,0,0) < eddy_y_max[1] + radii[4]){
     for (int i{0}; i < eddies[1]; i++){
       normpos[0] = (simulation_time - eddy_pos2_gbl[3*i] - eddy_r2_gbl[3*i]) / eddy_r2_gbl[3*i];
       normpos[1] = (x1_B0(0,0,0) - eddy_pos2_gbl[3*i+1]) / eddy_r2_gbl[3*i+1];
       normpos[2] = (x2_B0(0,0,0) - eddy_pos2_gbl[3*i+2]) / eddy_r2_gbl[3*i+2];
       ...
       if(rsq < 1.0){
         ...
         up += a11[idx[1]] * eddy_eps2_gbl[3*i] * shape[0];
         vp += (a21[idx[1]] * eddy_eps2_gbl[3*i] * shape[0] + a22[idx[1]] * eddy_eps2_gbl[3*i+1] * shape[1]);
         wp += a33[idx[1]] * eddy_eps2_gbl[3*i+2] * shape[2];
       }
       else{
         normz_ghost = (x2_B0(0,0,0) < 20.0) ? (x2_B0(0,0,0)+40.0 - eddy_pos2_gbl[3*i+2]) / eddy_r2_gbl[3*i+2] : (x2_B0(0,0,0)-40.0 - eddy_pos2_gbl[3*i+2]) / eddy_r2_gbl[3*i+2];
         rsq = rsq - normpos[2]*normpos[2] + normz_ghost*normz_ghost;
         if(rsq < 1.0){
           shape[0] = 1.0/1.4;//1.4;//1.7;
           shape[0] *= exp(-0.5*normpos[0]*normpos[0]);
   ...
   if(x1_B0(0,0,0) > eddy_y_min[2] - radii[7] && x1_B0(0,0,0) < eddy_y_max[2] + radii[7]){
     for(int i{0}; i < eddies[2]; i++){
       normpos[0] = (simulation_time - eddy_pos3_gbl[3*i] - eddy_r3_gbl[3*i]) / eddy_r3_gbl[3*i];
       normpos[1] = (x1_B0(0,0,0) - eddy_pos3_gbl[3*i+1]) / eddy_r3_gbl[3*i+1];
       normpos[2] = (x2_B0(0,0,0) - eddy_pos3_gbl[3*i+2]) / eddy_r3_gbl[3*i+2];
       ...
       if(rsq < 1.0){
         ...
         up += a11[idx[1]] * eddy_eps3_gbl[3*i] * shape[0];
         vp += (a21[idx[1]] * eddy_eps3_gbl[3*i] * shape[0] + a22[idx[1]] * eddy_eps3_gbl[3*i+1] * shape[1]);
         wp += a33[idx[1]] * eddy_eps3_gbl[3*i+2] * shape[2];
       }
       else{
         normz_ghost = (x2_B0(0,0,0) < 20.0) ? (x2_B0(0,0,0)+40.0 - eddy_pos3_gbl[3*i+2]) / eddy_r3_gbl[3*i+2] : (x2_B0(0,0,0)-40.0 - eddy_pos3_gbl[3*i+2]) / eddy_r3_gbl[3*i+2];
         rsq = rsq - normpos[2]*normpos[2] + normz_ghost*normz_ghost;
         if(rsq < 1.0){
           shape[0] = 1.0/1.4;
           shape[0] *= exp(-0.5*normpos[0]*normpos[0]);
   ...
   up = (std::abs(up) < 0.175) ? up : std::abs(up)/up * 0.175;
   vp = (std::abs(vp) < 0.175) ? vp : std::abs(vp)/vp * 0.175;
   wp = (std::abs(wp) < 0.175) ? wp : std::abs(wp)/wp * 0.175;

   Mlocal = Minf * u0/sqrt(T);
   Tp = (x1d > 0.0 && x1d < 1.0) ? -(gama-1)*Mlocal*Mlocal*up/u0*T : 0.0;
   rhop = -rho_temp * Tp / T;

   rho_B0(0,0,0) = rho_temp + rhop;

   rhou0_B0(0,0,0) = rho_B0(0,0,0)*(u0+up);

   rhou1_B0(0,0,0) = rho_B0(0,0,0)*(u1+vp);

   rhou2_B0(0,0,0) = rho_B0(0,0,0)*(u2+wp);
...
*rkA, const double *rkB)
{
 ...
 void opensbliblock00Kernel047(const ACC<double> &mu_B0, const ACC<double> &rhoE_B0, const ACC<double> &rho_B0, const
ACC<double> &rhou0_B0, const ACC<double> &rhou1_B0, const ACC<double> &rhou2_B0, ACC<double> &mu_mean_B0, ACC<double>
&rhoE_mean_B0, ACC<double> &rho_mean_B0, ACC<double> &rhohou0u0_mean_B0, ACC<double> &rhou0_mean_B0, ACC<double>
&rhou0u0_mean_B0, ACC<double> &rhou0u1_mean_B0, ACC<double> &rhou0u2_mean_B0, ACC<double> &rhou1_mean_B0, ACC<double>
&rhou1u1_mean_B0, ACC<double> &rhou1u2_mean_B0, ACC<double> &rhou2_mean_B0, ACC<double> &rhou2u2_mean_B0, ACC<double>
&u0_mean_B0, ACC<double> &u0u0_mean_B0, ACC<double> &u0u1_mean_B0, ACC<double> &u1_mean_B0, ACC<double> &u1u1_mean_B0,
ACC<double> &u2_mean_B0, ACC<double> &u2u2_mean_B0)
{
   double rho_inv = 0.0;
   ...
   rhohou0u0_mean_B0(0,0,0) = (rhou0_B0(0,0,0)*rhou0_B0(0,0,0)) + rhohou0u0_mean_B0(0,0,0);

   mu_mean_B0(0,0,0) = mu_B0(0,0,0) + mu_mean_B0(0,0,0);

   u0u0_mean_B0(0,0,0) = (rhou0_B0(0,0,0)*rhou0_B0(0,0,0))*(rho_inv*rho_inv) + u0u0_mean_B0(0,0,0);

   u0u1_mean_B0(0,0,0) = (rho_inv*rho_inv)*rhou0_B0(0,0,0)*rhou1_B0(0,0,0) + u0u1_mean_B0(0,0,0);

   u1u1_mean_B0(0,0,0) = (rhou1_B0(0,0,0)*rhou1_B0(0,0,0))*(rho_inv*rho_inv) + u1u1_mean_B0(0,0,0);

   u2u2_mean_B0(0,0,0) = (rhou2_B0(0,0,0)*rhou2_B0(0,0,0))*(rho_inv*rho_inv) + u2u2_mean_B0(0,0,0);

   u0_mean_B0(0,0,0) = rhou0_B0(0,0,0)*rho_inv + u0_mean_B0(0,0,0);

   u1_mean_B0(0,0,0) = rhou1_B0(0,0,0)*rho_inv + u1_mean_B0(0,0,0);

   u2_mean_B0(0,0,0) = rhou2_B0(0,0,0)*rho_inv + u2_mean_B0(0,0,0);

}

 void opensbliblock00Kernel048(ACC<double> &mu_mean_B0, ACC<double> &rhoE_mean_B0, ACC<double> &rho_mean_B0, ACC<double>
&rhohou0u0_mean_B0, ACC<double> &rhou0_mean_B0, ACC<double> &rhou0u0_mean_B0, ACC<double> &rhou0u1_mean_B0, ACC<double>
&rhou0u2_mean_B0, ACC<double> &rhou1_mean_B0, ACC<double> &rhou1u1_mean_B0, ACC<double> &rhou1u2_mean_B0, ACC<double>
&rhou2_mean_B0, ACC<double> &rhou2u2_mean_B0, ACC<double> &u0_mean_B0, ACC<double> &u0u0_mean_B0, ACC<double>
&u0u1_mean_B0, ACC<double> &u1_mean_B0, ACC<double> &u1u1_mean_B0, ACC<double> &u2_mean_B0, ACC<double> &u2u2_mean_B0,
ACC<double>& taux0x1_mean_B0, ACC<double>& l_mean_B0, ACC<double>& du0dx1_mean_B0, ACC<double>& utau_mean_B0)
{
   ...
   rhohou0u0_mean_B0(0,0,0) = invniter*rhohou0u0_mean_B0(0,0,0);
...
// new kernel: eval taux0_mean_B0. rhorhou0u0 is evaluated in kernel063, with the other averaging operations.

void opensbliblock00Kernel_new01(const ACC<double>& mu_B0, const ACC<double>& rho_B0, const ACC<double>& rhou0_B0, const ACC<double>& D11_B0,
ACC<double>& taux0x1_mean_B0, ACC<double>& l_mean_B0, ACC<double>& du0dx1_mean_B0, ACC<double>& utau_mean_B0, const int* idx){
double d1_u_dy = 0.0;
  ...
  utau_mean_B0(0,0,0) = utau_mean_B0(0,0,0) + sqrt(mu_B0(0,0,0)*D11_B0(0,0,0)*d1_u_dy/rho_B0(0,0,0));
